...
4. Komponenten-Code anpassen (`gateway.cpp`, `node_manager.cpp`, `cron.cpp`, `heartbeat.cpp`) und `#include "runtime_event_manager.hpp"` sowie die Helper-Funktionen nutzen (`makeGatewayStartedEvent`, `makeNodeConnectedEvent`, ...).
5. Listener im System-Init registrieren (z. B. in `main.cpp`).
6. Am Ende jeder Haupt-Loop-Iteration `openclaw::getDispatcher().dispatchAll();` aufrufen.

## Weiterführende Events

Zusätzlich zu den im Enum definierten Events können weitere Payload-Klassen für `EVENT_TASK_COMPLETED`, `EVENT_CRON_RUN_COMPLETED` usw. ergänzt werden, z. B. aus Abstraction-Manager, Cluster-Nodes, Github, Clawhub und TikTok.

## Hinweis

Dieses Skill-Proposal enthält Referenzen zu den Quell-Dateien im Haupt-Repository. Die eigentlichen Implementierungen bleiben dort erhalten; das Skill-Paket dient nur zur Wiederverwendung und Dokumentation.
