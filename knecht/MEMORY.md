...
<!-- openclaw-memory-promotion:memory:memory/2026-08-09.md:5:8 -->
- 21:42 CEST — heartbeat recovery check: Host diagnostics healthy: root disk 36% used; 11 GiB RAM available; CPU 97% idle.; Gateway journal recorded repeated OpenRouter billing 402 failures for heartbeat runs (requested token allowance exceeds available credits), followed by an unavailable `openai/gpt-5.6-sol` fallback. Earlier runs ultimately fell back successfully to `openai/gpt-5.6-terra`.; Required Gateway diagnostics could not be run because Gateway execution is disallowed in this session (`exec host not allowed`).; Attempt to inform `ops-hub` was blocked because its session is not visible from this sandboxed agent session. [score=0.812 recalls=0 avg=0.620 source=memory/2026-08-09.md:5-8]

## Promoted From Short-Term Memory (2026-08-16)

<!-- openclaw-memory-promotion:memory:memory/2026-08-10.md:5:7 -->
- 06:42 CEST — Heartbeat recovery check: Host diagnostics normal: root filesystem 36% used; 12 GiB RAM available; load average 0.97 / 0.40 / 0.28; vmstat showed 96% idle CPU and no swapping.; Gateway logs show recurring provider failures: OpenRouter `openrouter/auto` returns HTTP 402 (insufficient credits), and the first fallback `openai/gpt-5.6-sol` is unavailable to the configured project. Runs eventually fall through to `openai/gpt-5.6-terra` successfully.; Attempted to notify `ops-hub` via session send; delivery was denied because `agent:ops-hub:main` is not visible from this sandboxed session.... [score=0.812 recalls=0 avg=0.620 source=memory/2026-08-10.md:5-7]
<!-- openclaw-memory-promotion:memory:memory/2026-08-10.md:11:14 -->
- 15:42 CEST — Heartbeat recovery check: Host remains healthy: root filesystem 36% used; 11 GiB RAM available; load 1.02 / 0.41 / 0.28; no swapping and CPU was 86–96% idle.; The OpenRouter HTTP 402 / inaccessible `gpt-5.6-sol` fallback incident remains active. Gateway is reachable and its systemd service is active; the connected node is healthy.; `openclaw doctor --yes` and `--force` found a stale disabled `memory-lancedb` configuration, a Claude auth profile expiring in about 6 hours, 1 missing recent-session transcript, and 79 orphan main-session transcripts.... [score=0.812 recalls=0 avg=0.620 source=memory/2026-08-10.md:11-14]
<!-- openclaw-memory-promotion:memory:memory/2026-08-10.md:18:21 -->
- 21:43 CEST — Heartbeat recovery check: Host diagnostics remain healthy: root filesystem 36% used; 13 GiB RAM available; load 0.37 / 0.17 / 0.12; no swap activity and 96% idle CPU.; Gateway logs still show OpenRouter HTTP 402 billing/token-limit failures and an inaccessible `openai/gpt-5.6-sol` fallback. The eventual `openai/gpt-5.6-terra` fallback succeeds.; Prescribed `openclaw doctor --yes` and `--force` ran. Outstanding findings: expired Claude CLI auth, stale disabled `memory-lancedb` config, 1 missing main-session transcript, 87 orphan main-session transcripts, and plaintext secret-bearing config fields.... [score=0.812 recalls=0 avg=0.620 source=memory/2026-08-10.md:18-21]
