...
def generate_periodic_table_html(output_file):
    elements = [
...
'''
    ...
    html_content += 'const E = [\n'
    for element in elements:
        html_content += f"  [{element[0]},'{element[1]}','{element[2]}','{element[3]}','{element[4]}',{element[5]},{element[6]}],\n"
    html_content += '];\n'

    html_content += 'const CAT = {\n'
    for key, value in categories.items():
        html_content += f"  {key}:'{value}',\n"
    html_content += '};\n'

    html_content += 'const CATPL = {\n'
    for key, value in category_plural.items():
        html_content += f"  {key}:'{value}',\n"
    html_content += '};\n'
...
if __name__ == '__main__':
    ...
    generate_periodic_table_html(output_file)
