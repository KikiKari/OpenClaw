...
if ($args.Count -ne 1) {
    Write-Error "Aufruf: pwsh $($MyInvocation.MyCommand.Name) <Ausgabedatei>"
    exit 1
}
$outputPath = $args[0]
...
# Schreibe den HTML-Inhalt in die Ausgabedatei mit UTF-8 Codierung
[IO.File]::WriteAllText($outputPath, $htmlContent, [Text.Encoding]::UTF8)
