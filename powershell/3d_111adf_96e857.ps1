...
try {
    $html_content = Generate-Html
} catch {
    Write-Error "Error generating HTML: $_"
    exit 1
}

try {
    $html_content | Set-Content -Path $output_file -Encoding UTF8
} catch {
    Write-Error "Error writing to file '$output_file': $_"
    exit 1
}

Write-Host "HTML file generated successfully: $output_file"
