...
# Portiert nach PowerShell 7 am: 2024-12-19
  ...
  Object.keys(P).forEach(name=>{
    ...
    d.innerHTML=`<h3>${esc(name)} <button class="btn sm" data-delp="${esc(name)}">${tr("del")}</button></h3>${rows}
      <div class="row" style="margin-top:6px">
        <input data-newk="${esc(name)}" placeholder="${tr("newField")}" style="max-width:140px">
        <input data-newv="${esc(name)}" placeholder="${tr("newValue")}">
        <button class="btn sm" data-add="${esc(name)}">${tr("addField")}</button>
      </div>`;
    root.appendChild(d);
  });
  saveMsg.textContent=""; result.value="";
}
function vaultToObj(){ // collect current edits into VAULT object
  const P=VAULT.providers||{};
  document.querySelectorAll("[data-p]").forEach(el=>{ const p=el.dataset.p, k=el.dataset.k; if(P[p]&&P[p].hasOwnProperty(k)) P[p][k]=el.value; });
}
function msg(id,text,ok=true){ document.getElementById(id).textContent=text; document.getElementById(id).className="msg "+(ok?"ok":"err"); }

document.getElementById("newBtn").onclick=()=>{
  const pw=pass.value.trim(); if(!pw){ msg("openMsg",tr("needPass"),false); return; }
  VAULT={meta:{created:new Date().toISOString()},providers:{}};
  renderEditor(); msg("openMsg",tr("created"),true);
};
document.getElementById("openBtn").onclick=async()=>{
  const pw=pass.value.trim(); if(!pw){ msg("openMsg",tr("needPass"),false); return; }
  let b64=blob.value.trim();
  if(!b64 && file.files.length>0){
    const f=file.files[0]; const r=new FileReader();
    b64=await new Promise(res=>{ r.onload=()=>res(r.result.split(",")[1]); r.readAsDataURL(f); });
  }else if(!b64){ msg("openMsg",tr("noInput"),false); return; }
  try{
    VAULT=await decryptB64(b64,pw);
    renderEditor(); msg("openMsg",tr("opened"),true);
  }catch(e){ msg("openMsg",tr("bad"),false); }
};

document.getElementById("addProvBtn").onclick=()=>{
  if(!VAULT){ msg("saveMsg",tr("needOpen"),false); return; }
  const n=newProv.value.trim(); if(!n) return;
  (VAULT.providers||(VAULT.providers={}))[n]={}; newProv.value="";
  renderEditor();
};
document.getElementById("provs").addEventListener("click",e=>{
  if(!VAULT) return;
  const t=e.target;
  if(t.dataset.delp){
    delete VAULT.providers[t.dataset.delp]; renderEditor();
  }else if(t.dataset.del){
    const [p,k]=t.dataset.del.split("|"); delete VAULT.providers[p][k]; renderEditor();
  }else if(t.dataset.add){
    const p=t.dataset.add, ki=document.querySelector(`[data-newk="${esc(p)}"]`), vi=document.querySelector(`[data-newv="${esc(p)}"]`);
    const k=ki.value.trim(), v=vi.value.trim(); if(!k || !v) return;
    (VAULT.providers[p]||(VAULT.providers[p]={}))[k]=v; ki.value=""; vi.value="";
    renderEditor();
  }
});
...
document.getElementById("encBtn").onclick=async()=>{
  ...
  vaultToObj();
  try{
    result.value=await encryptObj(VAULT,pass.value);
    msg("saveMsg",tr("encrypted"),true);
  }catch(e){ msg("saveMsg","ERROR: "+e,false); }
};
document.getElementById("dlBtn").onclick=()=>{
  if(!result.value){ msg("saveMsg",tr("needOpen"),false); return; }
  const a=document.createElement("a");
  a.href="data:application/octet-stream;base64,"+result.value;
...
document.getElementById("expBtn").onclick=()=>{
  ...
  vaultToObj();
  const a=document.createElement("a");
  a.href="data:text/plain;charset=utf-8,"+encodeURIComponent(JSON.stringify(VAULT,null,2));
