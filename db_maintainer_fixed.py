#!/usr/bin/env python3
"""
Database Maintainer Sub-Agent (Fixed Workspace Path)
...
# Use the correct workspace path for this sandbox
...
class DatabaseMaintainer:
    ...
    def update_databases(self):
        try:
            result = subprocess.run(['python3', str(WORKSPACE / 'scripts' / 'update_docs_db.py')],
                                    capture_output=True, text=True, timeout=60)
            if result.returncode == 0:
    ...
    def update_tree_db_v2(self):
        try:
            result = subprocess.run(['python3', str(WORKSPACE / 'scripts' / 'tree_indexer_v2.py')],
    ...
    def cleanup_old_backups(self):
        ...
        for db_name in ['docs.db', 'tree.db']:
            for backup in BACKUP_DIR.glob(f"*_{db_name}.bak"):
                try:
                    date_str, time_str, _ = backup.name.split('_')
    ...
    def run_cycle(self):
        ...
        self.logger.info('Checking for documentation changes...')
        ...
        # Determine if backup is due (hourly)
