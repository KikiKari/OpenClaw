...
function createCSSVariables(vars) {
...
function generateCSS() {
  ...
  // Add imports
  css += cssStructure.imports.join('\n') + '\n\n';
  
  // Add :root variables
  css += ':root {\n' + createCSSVariables(cssStructure.root) + '\n}\n\n';
  
  // Add @theme inline
  css += '@theme inline {\n' + createCSSVariables(cssStructure.theme) + '\n}\n\n';
  
  // Add regular rules
  for (const [selector, properties] of Object.entries(cssStructure.rules)) {
    css += createRule(selector, properties) + '\n';
  }
  css += '\n';
  
  // Add media queries
  for (const [query, rules] of Object.entries(cssStructure.mediaQueries)) {
    css += query + ' {\n';
  ...
  // Add special rules
  css += cssStructure.specialRules.join('\n') + '\n\n';
  
  // Add keyframes
  css += createKeyframes('dropfall', cssStructure.keyframes.dropfall) + '\n';
...
function main() {
  ...
  const outputFile = args[0] || 'globals.css';
