...
function generateCell(el) {
  const [z, sym, name, mass, cat, row, col] = el;
  const gridRow = row >= 8 ? row - 7 : row;
  const escapedName = escapeHtml(name);
  const escapedSym = escapeHtml(sym);
  
  return `<button class="cell cat-${cat}" style="grid-column:${col};grid-row:${gridRow}" data-z="${z}" title="${escapedName} (${escapedSym})">
<span class="num">${z}</span><span class="sym">${escapedSym}</span>
</button>`;
}

// Generate marker cell
function generateMarker(r, cat, txt) {
  return `<div class="cell cell-marker cat-${cat}" style="grid-column:3;grid-row:${r}">${txt}</div>`;
}

// Generate legend item
function generateLegendItem(k, v) {
  return `<div class="legend-item"><span class="legend-swatch cat-${k}"></span><span>${v}</span></div>`;
}

// Generate detail view
function generateDetailView() {
  return '<div class="detail-empty">Klicke auf ein Element, um Details zu sehen</div>';
}

// Generate the complete HTML document
function generateHTML() {
  // Generate main grid cells
  let mainCells = '';
  let fblockCells = '';
  
  // Add marker cells
  mainCells += generateMarker(6, 'la', '57-71');
  mainCells += generateMarker(7, 'ac', '89-103');
  
  // Add element cells
  E.forEach(el => {
    const cell = generateCell(el);
    if (el[5] >= 8) {
      fblockCells += cell;
    } else {
      mainCells += cell;
  ...
  Object.entries(CATPL).forEach(([k, v]) => {
    legendItems += generateLegendItem(k, v);
  });
...
<div class="detail" id="detail">${generateDetailView()}</div>
<div class="grid" id="main">${mainCells}</div>
<div class="grid fgrid" id="fblock">${fblockCells}</div>
...
console.log(`Periodensystem HTML file generated: ${outputFile}`);
