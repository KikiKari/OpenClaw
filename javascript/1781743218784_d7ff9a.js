...
function renderEditor(){
  ...
  Object.keys(P).forEach(name=>{
    ...
    d.innerHTML=\`<h3>\${esc(name)} <button class="btn sm" data-delp="\${esc(name)}">\${tr("del")}</button></h3>\${rows}
      <div class="row" style="margin-top:6px"><input data-addk="\${esc(name)}" placeholder="\${tr("newField")}" style="max-width:160px"><input data-addv="\${esc(name)}" placeholder="\${tr("newValue")}"><button class="btn sm" data-add="\${esc(name)}">\${tr("addField")}</button></div>\`;
    root.appendChild(d);
  });
  document.querySelectorAll("input[data-p]").forEach(el=>el.oninput=e=>{VAULT.providers[el.dataset.p][el.dataset.k]=el.value;});
  document.querySelectorAll("button[data-del]").forEach(el=>el.onclick=e=>{const [p,k]=el.dataset.del.split("|"); delete VAULT.providers[p][k]; renderEditor();});
  document.querySelectorAll("button[data-delp]").forEach(el=>el.onclick=e=>{delete VAULT.providers[el.dataset.delp]; renderEditor();});
  document.querySelectorAll("button[data-add]").forEach(el=>el.onclick=e=>{
    const p=el.dataset.add, k=document.querySelector(\`input[data-addk="\${p}"]\`).value, v=document.querySelector(\`input[data-addv="\${p}"]\`).value;
    if(k&&v){VAULT.providers[p][k]=v; renderEditor();}
  });
}
newBtn.onclick=()=>{VAULT={meta:{created:new Date().toISOString()},providers:{}}; renderEditor(); openMsg.textContent=tr("created"); openMsg.className="msg ok";};
openBtn.onclick=async()=>{
  const pw=document.getElementById("pass").value;
  if(!pw){openMsg.textContent=tr("needPass"); openMsg.className="msg err"; return;}
  const file=document.getElementById("file").files[0], blob=document.getElementById("blob").value.trim();
  if(!file&&!blob){openMsg.textContent=tr("noInput"); openMsg.className="msg err"; return;}
  try{
    const b64=file?await file.text():blob;
    VAULT=await decryptB64(b64,pw);
    renderEditor();
    openMsg.textContent=tr("opened"); openMsg.className="msg ok";
  }catch(e){openMsg.textContent=tr("bad"); openMsg.className="msg err";}
};
encBtn.onclick=async()=>{
  ...
  }catch(e){saveMsg.textContent="Error: "+e.message; saveMsg.className="msg err";}
};
dlBtn.onclick=()=>{
  const a=document.createElement("a"); a.href="data:application/octet-stream;base64,"+document.getElementById("result").value;
  a.download="vault.svpb"; a.click();
};
expBtn.onclick=()=>{document.getElementById("result").value=JSON.stringify(VAULT,null,2);};
addProvBtn.onclick=()=>{const n=newProv.value.trim(); if(n&&!VAULT.providers[n]){VAULT.providers[n]={}; renderEditor();}};
</script>
</body>
</html>`;
