...
function renderHTML(node, indent = '') {
  ...
  if (node.type === 'element') {
    ...
    let content = '';
    const isScriptOrStyle = node.tag === 'script' || node.tag === 'style';
    
    if (isScriptOrStyle && node.children.length > 0) {
      content = node.children.map(child => renderHTML(child, '')).join('');
    ...
    content = '\n';
    node.children.forEach(child => {
      content += renderHTML(child, indent + '  ');
    });
    return `${indent}<${node.tag}${attrs}>${content}${indent}</${node.tag}>\n`;
  }
