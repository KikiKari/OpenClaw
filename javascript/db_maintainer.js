...
`;
    ...
    scanDocumentations() {
        ...
        const walk = (dir) => {
            ...
            for (const entry of entries) {
                ...
                } else if (entry.isDirectory() && !relativePath.includes('node_modules')) {
                    walk(fullPath);
                }
            }
        };
    ...
    createBackup() {
        ...
        const timestamp = new Date().toISOString().replace('T', '_').replace(/:/g, '-').substring(0, 16);
    ...
    runCycle() {
        ...
        if (lastBackup) {
            ...
            doBackup = (Date.now() - lastBackupTime.getTime()) >= 60 * 60 * 1000;
        } else {
            doBackup = true;
        }
