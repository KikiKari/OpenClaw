...
// Funktion zum Erstellen eines HTML-Elements mit Attributen und Kindern
function createElement(tag, attributes = {}, children = []) {
  const element = { tag, attributes, children };
  return element;
}

// Funktion zum Erstellen eines Textknotens
function createTextNode(text) {
  return { type: 'text', text };
}

// Funktion zum Erstellen des gesamten HTML-Dokuments
function createDocument() {
  ...
  head.children.push(createElement('title', {}, [createTextNode('Kontaktbogen — Project-Landingpage Assets')]));
  
  // Styles
  const style = createElement('style');
  style.children.push(createTextNode(`
...
`));
  ...
  const headerH1 = createElement('h1', {}, [createTextNode('Kontaktbogen — Project-Landingpage Bildbestand')]);
  const headerP = createElement('p', {}, [createTextNode('Alle vorhandenen HighRes-Bilder aus deiner Design-Referenz, dem laufenden Codex-Repo und den Anhängen. Kategorisiert nach Verwendungszweck. Bitte markiere pro Kategorie welche Assets in die finale Umsetzung sollen und wo du Erweiterung wünschst.')]);
  
  const legend = createElement('div', { class: 'legend' });
  legend.children.push(createElement('span', { class: 'pill pflicht' }, [createTextNode('PFLICHT — muss ins finale Design')]));
  legend.children.push(createElement('span', { class: 'pill auswahl' }, [createTextNode('AUSWAHL nötig — mehrere Kandidaten')]));
  legend.children.push(createElement('span', { class: 'pill' }, [createTextNode('Referenz — nur Ästhetik, nicht direkt verwendet')]));
  legend.children.push(createElement('span', { class: 'pill' }, [createTextNode('Vorhanden — schon im Repo eingebaut')]));
  
  header.children.push(headerH1);
  header.children.push(headerP);
  header.children.push(legend);
  ...
  categories.forEach(category => {
    const section = createElement('section', { id: category.id });
    const sectionH2 = createElement('h2');
    
    sectionH2.children.push(createTextNode(category.title + ' '));
    
    if (category.tag) {
      const tag = createElement('span', { class: `tag ${category.tag}` }, [createTextNode(category.tagText)]);
      sectionH2.children.push(tag);
      sectionH2.children.push(createTextNode(' '));
    }
    
    const countTag = createElement('span', { class: 'tag' }, [createTextNode(category.count)]);
    sectionH2.children.push(countTag);
    
    const sub = createElement('p', { class: 'sub' }, [createTextNode(category.subtitle)]);
    ...
    category.images.forEach(image => {
      ...
      const imgwrap = createElement('div', { class: 'imgwrap' });
      ...
      if (image.src) {
        ...
        imgwrap.children.push(img);
      } else {
        imgwrap.children.push(createElement('div', { style: 'color:#999' }, [createTextNode('kein Thumbnail')]));
      }
      
      const meta = createElement('div', { class: 'meta' });
      const name = createElement('div', { class: 'name' }, [createTextNode(image.name)]);
      const dims = createElement('div', { class: 'dims' }, [createTextNode(image.dims)]);
      const code = createElement('div', {}, [createTextNode(image.code)]);
      
      meta.children.push(name);
      meta.children.push(dims);
      meta.children.push(code);
      
      if (image.prompt) {
        const prompt = createElement('div', { class: 'prompt' }, [createTextNode(image.prompt)]);
        meta.children.push(prompt);
      }
      
      card.children.push(imgwrap);
      card.children.push(meta);
      
      grid.children.push(card);
    });
    
    section.children.push(sectionH2);
  ...
  return createElement('!DOCTYPE html', {}, [
    html
  ]);
}

// Funktion zum Serialisieren des DOM-Baums in HTML-String
...
function main() {
  ...
  let outputFile = 'kontaktbogen.html';
