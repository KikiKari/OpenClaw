...
function createLinkTag(attributes) {
  const attrs = Object.entries(attributes)
    .map(([key, value]) => `${key}="${value}"`)
    .join(' ');
  return `<link ${attrs} />`;
...
function createScriptTag(attributes) {
  const attrs = Object.entries(attributes)
    .map(([key, value]) => `${key}="${value}"`)
    .join(' ');
...
function generateHTML() {
  return createHeader() + createBanner() + `
    <div class="content-wrapper">
      <div class="content">` + createSidebar() + createMainContent();
}

// Main execution
if (process.argv.length !== 3) {
  console.error('Usage: node ' + process.argv[1] + ' <output_file>');
  process.exit(1);
}
