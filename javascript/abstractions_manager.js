...
`;
        ...
        log(`Failed: ${script_path} - ${e.message}`, "ERROR");
        return false;
    }
}
...
function create_status_report(state) {
    ...
    if (fs.existsSync(ABSTRACTIONS_REPO)) {
        ...
        for (const lang_dir of dirs) {
            const dirPath = path.join(ABSTRACTIONS_REPO, lang_dir);
            if (fs.statSync(dirPath).isDirectory() && TARGET_LANGUAGES[lang_dir]) {
                const files = fs.readdirSync(dirPath);
                lang_counts[lang_dir] = files.filter(f => fs.statSync(path.join(dirPath, f)).isFile()).length;
            }
        }
    }
    ...
    for (const [lang, count] of Object.entries(lang_counts).sort()) {
        content += `- ${lang}: ${count}\n`;
    }
    ...
    content += "|------|---------------|-----------|-----------|-------|\n";
    for (const [node_id, config] of Object.entries(NODES)) {
