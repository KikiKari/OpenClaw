...
{"name":"Secret Vault Public","schemaVersion":1,"description":"Secret-Vault Public als interaktives Browser-Artefakt: verschlüsselter Secret-Container vollständig client-seitig (WebCrypto, AES-256-GCM + PBKDF2). Öffnen/Anlegen, Anbieter/Felder ergänzen und ersetzen (Rotation), verschlüsseln und als .svpb herunterladen oder Klartext-JSON exportieren. DE/EN nach Browsersprache. Eigenes Format (nicht kompatibel mit dem scrypt-Python-Tool). Keine Secrets eingebettet.","mcpTools":[],"mcpServerNames":[]}
</script>
<html lang="de">
<head>
<meta charset="utf-8">
...
main() {
    # Hauptfunktion
    if [ $# -ne 1 ]; then
        echo "Verwendung: python script.py <ausgabedatei.html>"
        exit 1
    fi
