...
# Function to generate the specification data as JSON
get_spec() {
    cat <<'EOF'
{
  "schichten": [
...
generate_html() {
    ...
    # Escape quotes and backslashes for safe inclusion in HTML
    spec_json_escaped=$(printf '%s' "$spec_json" | sed 's/\\/\\\\/g; s/"/\\"/g')
