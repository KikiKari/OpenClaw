...
set options {
    {OutputPath.arg "" "The path where to save the HTML file"}
}

# Parse command line arguments
array set opts [cmdline::getoptions argv $options]
set OutputPath $opts(OutputPath)

if {$OutputPath eq ""} {
    puts stderr "Error: OutputPath argument is required"
    exit 1
}
...
if {[catch {set fileId [open $OutputPath w]} errorMsg]} {
    puts stderr "Error opening file for writing: $errorMsg"
    exit 1
}

if {[catch {puts -nonewline $fileId $html_content} errorMsg]} {
    puts stderr "Error writing file: $errorMsg"
    close $fileId
    exit 1
}
