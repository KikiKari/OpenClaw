    ...
    set SPEC [dict create \
        schichten [list \
            [dict create \
                ...
                farbe "#5f6773" \
                blocks [list \
                    [dict create id "mcp-domain" name "mcp.DOMAIN" untertitel "Streamable HTTP"] \
                    [dict create id "docs-mcp" name "docs/mcp" untertitel "Anbieterdoku"] \
                    [dict create id "well-known" name ".well-known" untertitel "OAuth-Metadaten"] \
                    [dict create id "config-json" name "config.json" untertitel "claude_desktop_config"] \
                ] \
            ] \
            [dict create \
                name "Sonde" \
                farbe "#2481cc" \
                blocks [list \
                    [dict create id "discovery-py" name "discovery.py" untertitel "sechs Pfade"] \
                    [dict create id "config-py" name "config.py" untertitel "MSIX-Falle"] \
                ] \
            ] \
            [dict create \
                name "Klassifikation" \
                farbe "#6d5bd0" \
                blocks [list \
                    [dict create id "state-py" name "state.py" untertitel "fuenf Zustaende"] \
                ] \
            ] \
            [dict create \
                name "Ausgabe" \
                farbe "#15803d" \
                blocks [list \
                    [dict create id "report-py" name "report.py" untertitel "Textausgabe"] \
                    [dict create id "server-py" name "server.py" untertitel "127.0.0.1"] \
                    [dict create id "index-html" name "index.html" untertitel "statische Seite"] \
                ] \
            ] \
        ] \
        kanten [list \
            [dict create von "mcp-domain" nach "discovery-py" art "fluss"] \
            [dict create von "docs-mcp" nach "config-py" art "fluss"] \
            [dict create von "well-known" nach "discovery-py" art "fluss"] \
            [dict create von "config-json" nach "config-py" art "fluss"] \
            [dict create von "discovery-py" nach "state-py" art "fluss"] \
            [dict create von "config-py" nach "state-py" art "fluss"] \
            [dict create von "state-py" nach "report-py" art "fluss"] \
        ] \
        kantenarten [list \
            [dict create art "fluss" farbe "#6d5bd0" stil "voll" text "Fluss von unten nach oben"] \
        ] \
    ]
...
(function(){
  "use strict";
  var SPEC = }[json::write write $SPEC][{;
