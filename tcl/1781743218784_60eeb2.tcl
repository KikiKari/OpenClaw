...
proc main {argv} {
    ...
    if {[catch {set htmlContent [generateHTML]}]} {
        puts stderr "Error generating HTML content"
        exit 1
    }
    
    if {[catch {set fh [open $outputFile w]}]} {
        puts stderr "Error opening file for writing: $outputFile"
        exit 1
    }
    
    if {[catch {puts $fh $htmlContent; close $fh}]} {
        puts stderr "Error writing to file: $outputFile"
        exit 1
    }
