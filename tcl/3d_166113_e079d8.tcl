...
# Tcl/Tk port of the 3D architecture visualization for TikTok LIVE Companion Android

package require Tk

# Procedure to generate the complete HTML content as a string
proc generate_html {} {
    return {<!DOCTYPE html>
<html lang="de">
<head>
...
# Main procedure to write the HTML file
proc main {argv} {
    if {[llength $argv] != 1} {
    ...
    if {[catch {open $output_file w} f]} {
        puts "Error opening file $output_file: $f"
        exit 1
    }
    
    if {[catch {puts -nonewline $f $html_content} error]} {
        puts "Error writing to file $output_file: $error"
        close $f
        exit 1
    }
    
    if {[catch {close $f} error]} {
        puts "Error closing file $output_file: $error"
        exit 1
    }
