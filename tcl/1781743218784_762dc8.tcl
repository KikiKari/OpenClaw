...
proc dictToJson {dict_val} {
    set json "{"
    set first 1
    foreach {key val} $dict_val {
        if {!$first} {
            append json ","
        }
        set first 0
        ...
        # Escape key
        set escaped_key $key
        regsub -all {(["\\])} $escaped_key {\\\1} escaped_key
        regsub -all {\n} $escaped_key "\\n" escaped_key
        regsub -all {\r} $escaped_key "\\r" escaped_key
        regsub -all {\t} $escaped_key "\\t" escaped_key
        
        append json "\"$escaped_key\":"
        
        # Handle value based on type
        if {[string is list $val] && [llength $val] > 1} {
            # Complex nested structure
            append json [dictToJson $val]
        } elseif {[string is list $val]} {
            # Simple list/array
            append json "\[[join $val ", "]\]"
        } else {
            # String value - escape it
            set escaped_val $val
            regsub -all {(["\\])} $escaped_val {\\\1} escaped_val
            regsub -all {\n} $escaped_val "\\n" escaped_val
            regsub -all {\r} $escaped_val "\\r" escaped_val
            regsub -all {\t} $escaped_val "\\t" escaped_val
            
            append json "\"$escaped_val\""
        }
    }
    append json "}"
    return $json
