...
array set SPEC {}
set SPEC(schichten) [list \
    [list name "Quelle" farbe "#5f6773" blocks [list \
        [list id "tiktok-webview" name "TikTok-WebView" untertitel "Hauptframe"] \
        [list id "www-tiktok-com" name "www.tiktok.com" untertitel "nur diese Origin"] \
        [list id "hauptframe" name "Hauptframe" untertitel "kein iframe"] \
    ]] \
    [list name "Bruecke" farbe "#2481cc" blocks [list \
        [list id "mobile-bridge-v1" name "Mobile Bridge v1" untertitel "Origin + Typ"] \
        [list id "origin-pruefung" name "Origin-Pruefung" untertitel "fail closed"] \
        [list id "typ-groesse" name "Typ + Groesse" untertitel "begrenzt"] \
    ]] \
    [list name "App" farbe "#6d5bd0" blocks [list \
        [list id "kotlin" name "Kotlin" untertitel "Sprache"] \
        [list id "jetpack-compose" name "Jetpack Compose" untertitel "Oberflaeche"] \
        [list id "androidx-webkit" name "AndroidX WebKit" untertitel "WebView"] \
    ]] \
    [list name "Audio" farbe "#b45309" blocks [list \
        [list id "shazamkit" name "ShazamKit" untertitel "nur nach Klick"] \
        [list id "mikrofon" name "Mikrofon" untertitel "stabil"] \
        [list id "webview-pcm-exp" name "WebView-PCM (exp.)" untertitel "experimentell"] \
    ]] \
    [list name "Ausgabe" farbe "#fe2c55" blocks [list \
        [list id "fluechtiger-streamzustand" name "fluechtiger Streamzustand" untertitel "nicht persistiert"] \
        [list id "panel" name "Panel" untertitel "Anzeige"] \
        [list id "apk" name "APK" untertitel "Android-Artefakt"] \
    ]] \
]

set SPEC(kanten) [list \
    [list von "tiktok-webview" nach "mobile-bridge-v1" art "fluss"] \
    [list von "www-tiktok-com" nach "origin-pruefung" art "fluss"] \
    [list von "hauptframe" nach "typ-groesse" art "fluss"] \
    [list von "mobile-bridge-v1" nach "kotlin" art "fluss"] \
    [list von "origin-pruefung" nach "jetpack-compose" art "fluss"] \
    [list von "typ-groesse" nach "androidx-webkit" art "fluss"] \
    [list von "kotlin" nach "shazamkit" art "fluss"] \
    [list von "jetpack-compose" nach "mikrofon" art "fluss"] \
    [list von "androidx-webkit" nach "webview-pcm-exp" art "fluss"] \
    [list von "shazamkit" nach "fluechtiger-streamzustand" art "fluss"] \
    [list von "mikrofon" nach "panel" art "fluss"] \
    [list von "webview-pcm-exp" nach "apk" art "fluss"] \
]

set SPEC(kantenarten) [list \
    [list art "fluss" farbe "#fe2c55" stil "voll" text "Fluss von unten nach oben"] \
]
...
proc to_json {data} {
    ...
    # Prüfen ob es ein Dictionary ist
    ...
    if {[llength $data] > 1 || [string index $data 0] eq "\["} {
        set json "\["
        set first 1
        foreach item $data {
            if {!$first} {
