#!/usr/bin/env python3
"""
Modified ClawHub ↔ Git Sync Agent for sandbox environment.
Uses sandbox paths for skills, git, and state.
"""
...
# Adjusted paths for sandbox
CLAWHUB_DIR = Path("/workspace/skills")
GIT_DIR = Path("/workspace/git/skills")
STATE_FILE = Path("/workspace/db/sync_state.json")
...
def get_all_skills():
    ...
    clawhub_skills = {d.name for d in CLAWHUB_DIR.iterdir() if d.is_dir() and not d.name.startswith('.') and d.name not in RESERVED_SKILL_NAMES and (d / "SKILL.md").exists()}
    git_skills = {d.name for d in GIT_DIR.iterdir() if d.is_dir() and not d.name.startswith('.') and d.name not in RESERVED_SKILL_NAMES and (d / "SKILL.md").exists()}
...
def sync_skill_bidirectional(skill_name: str):
    ...
    if clawhub_path.exists() and git_path.exists():
        ...
        if git_changes and not clawhub_changes:
            ...
            return "error"
        # Both have changes – choose newer based on mtime
        log(f"Content difference detected for: {skill_name}")
...
def main():
    ...
    results = {"synced_to_git": [], "synced_to_clawhub": [], "updated_git": [], "updated_clawhub": [], "no_change": [], "errors": []}
    for skill in sorted(all_skills):
        try:
    ...
    if "sync_history" not in state:
        state["sync_history"] = []
    state["sync_history"].append({"timestamp": datetime.now().isoformat(), "results": results})
