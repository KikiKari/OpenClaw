...
Für Playwright/Chromium-Automation auf Headless-Servern ist **Xvfb** (X Virtual Framebuffer) erforderlich:

| Node | Xvfb-Status | Installationspfad |
|------|-------------|-------------------|
| Node 2 (Netcup/Ubuntu) | ✅ Installiert | `/usr/bin/Xvfb` |
| Node 3 (xNetX/CentOS) | ✅ Installiert | `/usr/bin/Xvfb` |
...
**Hinweis Node 3:** Tailscale überschreibt `/etc/resolv.conf` — für yum externe DNS (`8.8.8.8`) verwenden.
...
sudo ufw allow out 853/tcp   # DNS over TLS (DoT)
