...
# Read the specification from DATA section or command line argument
my $spec_file = $ARGV[0] || \*DATA;
my $spec_json = do {
    local $/;
    <$spec_file>;
};
close $spec_file if $spec_file ne \*DATA;
...
# Output the specification as JavaScript variable
print "  var SPEC = " . encode_json($spec) . ";\n\n";
