#!/usr/bin/env perl
# 1781743218784_0e4dc0_4857ff_c17f4b.ps1 — portiert nach perl5
# Quelle: powershell, Projects@abstractions:powershell/1781743218784_0e4dc0_4857ff_c17f4b.ps1
...
=head1 SYNOPSIS
    ...
    Generiert eine HTML-Datei mit einer Secret Vault Public Web-Anwendung.

=head1 DESCRIPTION

    Dieses Skript generiert eine eigenständige HTML-Datei, die eine clientseitig
    verschlüsselte Secret-Vault mit WebCrypto-APIs implementiert (AES-256-GCM + PBKDF2).

=head1 PARAMETER

    OutputFile
        Der Pfad, unter dem die generierte HTML-Datei gespeichert wird.

=head1 EXAMPLE

    perl script.pl secret-vault.html
...
eval {
    ...
    write_file($output_file, encode('UTF-8', $html_content));
    print "HTML file generated: $output_file\n";
    1;
} or do {
    my $error = $@ || 'Unknown error';
    print STDERR "Failed to generate HTML file: $error\n";
    exit 1;
};
