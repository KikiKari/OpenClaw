...
sub check_ssh {
    ...
    my $cmd = "ssh -o ConnectTimeout=10 -o BatchMode=yes ${user}\@${host} 'echo \"OK\"' 2>/dev/null";
    my $output = `$cmd`;
    my $exit_code = $? >> 8;
...
sub send_alert {
    my ($alert) = @_;
    eval {
        my $cmd = "python3 $WORKSPACE/skills/channel-status-agent/scripts/channel_status.py --type alert --message '" . $alert->{"level"} . ": " . $alert->{"message"} . "' 2>/dev/null";
        system($cmd);
...
sub main {
    ...
    for (my $i = 0; $i < @nodes_to_check; $i += 2) {
