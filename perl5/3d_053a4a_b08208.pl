...
HTML_END
    ...
    open(my $fh, '>:encoding(UTF-8)', $output_path) or die "Kann Datei '$output_path' nicht öffnen: $!";
    print $fh $html_content;
    close($fh);
}

sub main {
    # Hauptfunktion zur Generierung der HTML-Datei.
    if (@ARGV != 1) {
        print "Verwendung: perl 3d.pl <Ausgabedatei>\n";
        exit(1);
    }
    
    my $output_path = $ARGV[0];
    generate_html($output_path);
    print "Generierte 3D-Visualisierungs-HTML-Datei: $output_path\n";
}
